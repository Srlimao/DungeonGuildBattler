...
let combatNetworkModule = null;
export function registerCombatNetwork(mod) {
  combatNetworkModule = mod;
}

export const LobbyNetwork = {
  isMock: () => isMockEnvironment,
  getMockPlayers: () => mockLobbyPlayers,
      ...
      setTimeout(() => {
        ...
        if (p) {
          ...
          if (localHero && !localHero.isHost) {
            ...
            if (allReady) {
              setTimeout(() => {
                if (combatNetworkModule) {
                  combatNetworkModule.startCombat();
                }
              }, 1000);
            }
          }
